...
const INTERVAL_CONFIG: Record<string, { seriesId: string; windowMin: number; atrKlineInterval: string }> = {
  "15": { seriesId: "10192", windowMin: 15, atrKlineInterval: "15m" },
  "5":  { seriesId: "10684", windowMin: 5,  atrKlineInterval: "5m"  },
};
...
async function fetchLiveEvents(seriesId: string): Promise<Market[]> {
  try {
    const url = `${GAMMA_BASE}/events?series_id=${seriesId}&active=true&closed=false&limit=25`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    const data = await res.json();
...
export async function GET(
  ...
  if (!config) {
    return NextResponse.json(
      { error: `Invalid interval "${interval}". Supported: ${Object.keys(INTERVAL_CONFIG).join(", ")}` },
      { status: 400 },
    );
  }

  try {
    const windowMs = config.windowMin * 60_000;
    ...
    const [btcPrice, events] = await Promise.all([
      fetchBinancePrice(),
      fetchLiveEvents(config.seriesId),
    ]);
    ...
    const [upPrice, downPrice, btcKlines1s, polyHistory, btcKlinesAtr] = await Promise.all([
      ...
      upTokenId
        ...
        : Promise.resolve([]),
      fetchBinanceKlines(config.atrKlineInterval, 30),
    ]);

    const atr = calculateAtr(btcKlinesAtr);
